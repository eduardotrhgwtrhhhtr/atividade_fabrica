use metflix;
select nome, data_nasc, loc_nasc from ator;
describe ator;
select * from ator;

insert into ator values("sqllx","2005-09-23","campogrande");
select * from ator where loc_nasc="campogrande";
...
select * from ms where "Campo Grande";
select max(vitima) from ms;
select min(vitima) from ms;

select * from ms where vitima = (select min(vitima) from ms);
select count(regiao) from ms;      #registros (contador)
select * from ms where vitima between 1 and 9;
select * from ms limit 5;          #tanto de registros que eu quero que aparece
select * from ms where vitima like'r%';
